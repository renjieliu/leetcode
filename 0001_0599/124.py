...
#         self.right = right
class Solution:
    def maxPathSum(self, root: 'Optional[TreeNode]') -> int: # O( N | 1 )
        def helper(output, node): # max can be achieved from curr node 
            if node:
                A = helper(output, node.left)
                B = helper(output, node.right)
                output[0] = max(output[0], A + node.val + B, node.val, A + node.val, B + node.val) #curr, left+curr+right, curr+left, curr+right
                return max(A + node.val, B + node.val, node.val) #choose left, right, or just current node
            else:
                return 0
