    ...
    def maximalRectangle(self, matrix: 'List[List[str]]') -> int:
        ...
        for r in range(len(matrix)):
            ...
            for c in range(len(matrix[0])):
                if matrix[r][c] == "0":
                    dp[-1].append([[0,0]]) # [left, up]
                else:
                    if r == 0:
                        if c == 0:
                            dp[-1].append([[1,1]]) 
                        else:
                            dp[-1].append([[dp[r][c-1][0][0] + 1 , 1]])
                    else:
                        if c == 0:
                            dp[-1].append([[1, dp[r-1][c][0][1] + 1]])
                        else:
                            curr_left = dp[r][c-1][0][0]+1 # first one is the max left, as it was added first
                            curr = []
                            curr.append([curr_left,1]) #start from the current line
                            for left, up in dp[r-1][c]:
                                curr.append( [min(left, curr_left), up + 1] ) #follow the previous columns cnt,  but row cnt +1 
                            dp[-1].append(curr)
            
                for left, up in dp[-1][-1]:
                    output = max(output, left*up)
        return output
