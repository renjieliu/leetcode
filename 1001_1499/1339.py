...
#         self.right = right
class Solution:
    def maxProduct(self, root: 'Optional[TreeNode]') -> int: # O( N | N )
        def dfs(subtotal, node):
            if node:
                A = dfs(subtotal, node.left) # total for left child
                B = dfs(subtotal, node.right) # total for right child
                subtotal.append([A, B])
                return A + node.val + B # total sum of current tree
            return 0 
    
        subtotal = []
        total = dfs(subtotal, root)
        mx = -float('inf')
        for left, right in subtotal: 
            mx = max(mx, (total - left)*left, (total - right)*right) # cut the left subtree or right subtree
        return mx % (10**9+7)
