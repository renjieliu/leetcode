class Solution:
    def countBadPairs(self, nums: 'List[int]') -> int: # O( N | N )
        ...
        N = len(nums)
        total = gauss(N-1) #total pairs. [1,2,3] ==> (1,2), (1,3), (2,3)
        dp = defaultdict(lambda: 0)
        good = 0 # j- i != nums[j] - nums[i] ==> nums[j]-j = nums[i]-i
        for i, n in enumerate(nums):
            good += dp[n-i] # if current delta is X, all the previous delta X can be combined with current one.
            dp[n-i] += 1 
        return total - good
